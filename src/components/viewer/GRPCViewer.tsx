  ...
  return (
    <div className="space-y-6">
      ...
      <div className="pb-6 border-b">
        <h1 className="text-2xl font-bold text-gray-900">{api.service_name}.{api.method_name}</h1>
      </div>
      ...
      {requestMessage.length > 0 && (
        <div>
          <h2 className="text-sm font-medium text-gray-500 uppercase tracking-wide mb-3">Request Message</h2>
                ...
                {requestMessage.map((field, i) => (
                  <tr key={i} className="border-t border-gray-200">
                    ...
                    <td className="px-4 py-2">
                      <span className="px-2 py-1 bg-orange-100 text-orange-700 rounded text-xs">{field.type}</span>
                    </td>
                    <td className="px-4 py-2">
                      {field.label && (
                        <span className={`px-2 py-1 rounded text-xs ${
                          field.label === 'required' ? 'bg-red-100 text-red-700' :
                          field.label === 'repeated' ? 'bg-blue-100 text-blue-700' :
      ...
      {responseMessage.length > 0 && (
        <div>
          <h2 className="text-sm font-medium text-gray-500 uppercase tracking-wide mb-3">Response Message</h2>
          <div className="bg-gray-50 rounded-lg overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-100">
                <tr>
      ...
      {api.proto_definition && (
        <div>
          <h2 className="text-sm font-medium text-gray-500 uppercase tracking-wide mb-3">Proto Definition</h2>
      ...
      {examples.length > 0 && (
        <div>
          <h2 className="text-sm font-medium text-gray-500 uppercase tracking-wide mb-3">Code Examples</h2>
              ...
              <div key={i} className="border border-gray-200 rounded-lg overflow-hidden">
                <div className="px-4 py-3 bg-gray-50 border-b flex items-center justify-between">
                  <div>
                    ...
                    <span className="ml-3 font-medium text-gray-900">{example.description || 'Example'}</span>
                  </div>
                </div>
                <CodeBlock code={example.code} language={example.language.toLowerCase() || 'javascript'} />
              </div>
            ))}
          </div>
        </div>
