    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      {loading ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mt-4">Loading APIs...</p>
        </div>
      ) : filteredAPIs.length === 0 ? (
        <div className="text-center py-12 bg-muted rounded-lg border border-dashed border-border">
          <p className="text-muted-foreground">No APIs found. Create your first API!</p>
        </div>
      ) : (
        <div className="space-y-2">
          {filteredAPIs.map((api) => (
            <div
              ...
              className="bg-card rounded-lg shadow-sm p-4 border border-border hover:shadow-md transition-shadow cursor-pointer"
              onClick={() => handleEdit(api)}
            >
              <div className="flex items-center gap-4">
                <MethodBadge method={getMethodForAPI(api) as any} />
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-foreground truncate">{api.displayName}</h3>
                  <p className="text-sm text-muted-foreground truncate font-mono">{api.displaySub}</p>
                </div>
